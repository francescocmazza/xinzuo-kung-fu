---
source_hash: becf1bd8b6d8c9f3297ee99913858e7faddb7d1cfa13a232c4123ccf141f0cbc
---

# 刀形及其用途

刀形是厨师和所要完成的工作之间的最直接的连接。 钢、 热处理和构造决定了边缘的行为, 但轮廓决定了手部是如何接近食物的。 长度设置了有用的中风; 高度创造了指针清除; 弯曲表明刀片是很容易通过一个成分还是可以忍受更辛苦的工作。 要点可以是精密的仪器, 安全的圆端或者几乎完全不存在。

因此,本章接近指南的开头。 在首先决定刀子必须做什么之后,更容易理解冶金。 厨师寻找一种日常工具、一种长片烤肉、一种鱼的柔性刀片或一架重型直升机正在解决四个不同的问题。 钢级或装饰模式都不能将错误的形状变成正确的工具。

以下表格是按工作而不是按国籍排列的。 每一个名字后面都有一个完整的 Xinzuo 示例, 这样解释就不会脱离配置。 照片可以识别宽度, 而不是所有可能的版本: 以相同名称出售的刀子在长度、 曲线、 厚度、 平衡和贝vel 几何上仍然会有所不同 。 这些细节必须在单个模型上检查 。

## 形状如何引导运动

弯曲的腹部会改变手动到达食物的方向。 在划线或摇动时, 弯曲边缘的工作部分会与旅行方向相对应。 因此, 由手提供的前向移动可以伴有在接触点的递进向下部分。 曲线不会产生额外的能量, 也不会使边缘本身更锐利: 它会帮助运动中的一部分进入腹部, 而接触点会沿着腹部前进 。

受宠若惊的边缘表现不同。 它使得完成推切或近乎垂直的切口更容易与大板接触,并给手动和边缘运动之间带来更直接的线性关系。 长窄的刀片会让食物在抽取切片时处于更大的边缘,并减少由高尖刀锋面造成的拖力。 一个狭窄的点会切入关节和小细节,而宽广、支持的脚跟则更适合控制力。

这些是趋势,而不是僵硬的定律。 陀螺可以有足够的曲线来舒缓岩石;西方厨师的刀子可以相对平坦;刀形的刀子可以是薄的通用刀具,而不是骨头工具。 有用的习惯是阅读实际的刀片从脚跟到脚尖,而不是只相信类别名称。

### 地表救济:空心和锤子

一些Xinzuo刀在刀片面上使用空洞、酒窝或锤子表面,这些特征打断了食物和钢材之间的广泛、连续接触。作为切片的弯曲,被悬浮的区域会留下小缝隙,食物和刀片面之间会留下空气。通过减少持续的表面接触和吸吸,这些小气袋可以更容易地释放湿润、星光或软质食物。

其效果不应作为食物永不粘合的保证提出,它取决于食物的深度和分布、刀片的完成、成分的湿度和质地以及切割技术。 同样的原则适用于是否将食物作为普通的Granton式的空洞或作为锤子的质地。

## 目录形状引用

以下引用取自 Xinzuo 目录主索引。 它是一个视觉定向工具。 随后使用完整 Xinzuo 图像进行个别讨论, 以便无法从刀形中分离任何解释 。
...
看看从脚跟到脚尖的边缘。它的连续曲线可以回顾船体的横截面。在划线或摇晃中,手部会向前移动,而弯曲腹部的倾斜工作部分会自然地将这一移动的部分引向食物中去。 实际的优势并不是,一个弯曲的刀子所独有的边缘部分——任何划线都使用边缘的相接部分——但曲线的地理测量法使累进渗透成为划线的自然部分**,而不是要求纯粹的垂直推动。

这在肉类中特别有用,因为肉类的切片可以跨过肌肉纤维、连接组织、膜和耐药性变化的领域。 切片运动和累进渗透的结合有助于边缘在这些结构中继续下去,而不必仅仅依赖向下压力。 这也是传统厨师的剖面图对肉类特别有效的原因之一,包括硬质或连接质富集的部分,同时保持其多功能性,足以用于一般的准备。

赋予许多西方厨师刀具权力的同样大工程也造成了限制。 与较薄、较窄的刀片相比,较宽和通常厚的刀片一旦已经深入食物** 便不易重新定向。 切割期间改变或细微地纠正切割角度需要更多努力,因为更多的刀片面和截面已经投入使用。 因此,厨师的刀子是多用途的,但并不是每一个细微的方向校正最灵活的几何法。

它仍然是一个精细的切割工具,而不是一个骨头直升机。 不要用坚硬的骨头击打它,用冷冻的食物扭动它,或者用它来杠杆。
...
曲线变小会改变刀子通过食物的方式。 它通常提供** 不包括由弯曲的腹部产生的自然渗透助力,** 因为前向运动在接触点没有被重新引导到向下部分。 因此, 陀螺在推力切割和长画切片期间会感到更加中和和和线性。 方向调整通常比较容易,但其几何法与传统的西方厨师刀子相比,它也相对没有( ) 专门面向肉类加工。

这并不会使吉奥托不适合肉类。 它仍然是一支非常多用途的刀,对无骨肉、鱼、蔬菜和一般准备效果很好。 区别在于:西方厨师的刀子使用更弯曲的手法来鼓励穿透、摇动或扫荡行动,而吉奥托通常用其中某些行为来交换平滑板接触和直接控制。 实际模型仍然不同,因此比较真正的曲线、高度、厚度和平衡,而不是仅仅依靠名字。
...
它的特征是跌落或羊脚般的鼻子也改变了叶刀头周围的钢材数量**。一个尖尖的西方厨师的刀子在接近尖尖时逐渐缩小,而一个双层则故意切除更多的材料,以创造其细角K-tip。一个桑托库通常在脊椎与边缘交接之前保持一个更宽、更完整的前部部分。这并不意味着每个桑托库的整体比每个厨师的刀子都更重——许多刀子实际上更短,更轻,但确实意味着** 本地的尖塔区域相对重要**。

这个更完整的前方赋予了它一种不同的权威。 支持鼻子的材料比一个非常精细的厨师点或K- 提普 中的材料要多, 所以提示区域一般不那么微妙, 更适合以板为方向的反复推动切割和已知形状的短垂直切片。 向前方保留的额外质量也可以给刀片带来更多的惰性, 帮助锋片在移动后继续下行。 但是, 它不应该被描述为自动产生更大的切割力或更容易的渗透力: 锐利、 边缘角、 厚度以及食物接触的几何形状仍然是决定性的。 较轻、 较薄、 更锋利的刀片可以通过比更重、 厚的刀片段更容易通过一个成分。

这种差异在床边变得特别明显。 床头将一些Santoko的材料在前面换成一个更容易准确定位、得分和重新定向的精细K-tip。 床头将一些点精确度换成一个更完整和更有支持的鼻子。 两者都本能地更好; 每一个选择都把质量和几何方法放在不同种类的工作从中得益的地方。 对这一权衡的更广泛讨论继续到 ** 体重和平衡:为什么 Heavier不是更好或更坏** 。

高度产生指关节通关和宽阔的向导面。 它对于蔬菜、水果、草药和肉类或鱼的无骨部分来说特别舒适。 它的主要局限在于其紧凑性:短边缘需要通过大烤肉或甜瓜做更多的划线,而非常平坦的松木则不是为夸张的岩石动作设计的。
...
这种几何方法特别有用,因为双层刀叶仍然比较大,很高。 一把大刀通常会觉得不太适合微妙的尖点工作,但K-提普给了用户一个精密的精确工具,而不会放弃正常板面准备所需的刀高。 从这个意义上讲,双层刀片将大量通用的刀片与更经常与小刀相联的尖片结合起来。

边缘通常是相当直的,所以双层奖励可以推切和切切。 精确的细微的提示也可以被扭曲、操纵和与板块的硬接触所伤害。
...
其结果因空洞的成分、尾部、深度和位置而异,因此,刀子不应被描述为普遍没有棍子。 尖锐的边缘保持平坦,并像另一个厨师的刀子一样锋利,而不会不必要地稀释空洞。
...
Nakiri 并非全部完成完全相同的前方。 一个重要家族有一个 \\ 坚固的四舍五入, 未拆卸的前角**。 以这种形式, 四舍五入的前方可以作为控制支持点停留在板上, 而刀子通过短弧进行细细细的细细细的细细的细细的细细的细细的细切。 前方不是要穿孔的; 其四舍五入的几何法有利于安全板的接触和重复移动 。

另一家族的刀锋更远地伸向刀锋** 。这一设计旨在使刀锋中更大比例的人参与切割,使用户几乎能够将有用的切割行动放在前面,而不是将该地区主要视为一种支持或过渡。

这些并不是两个僵硬的类别。 实际上,它们之间有许多分级:一些纳基里人鼻子宽,另一些只有软化的角落,而另一些则接近几乎平方但仍然尖锐的前方。 实际的剖面告诉用户,前线主要是一个支点支持、一个活跃的切割区或者两者之间的折中。 在每一种情况下,纳基里人从细薄的蔬菜导向的几何学中获得切除的方便度,而不是用于骨骼、冷冻食品或撞击。
...
它也是最难学的厨房刀之一** 。 高度、前方质量和缺乏熟悉的厨师刀形剖面改变了切割时使用的视觉和触觉参考。 初学者最初可能发现角度和细节的细微变化不太直观。

然而,在经验丰富的人手中,同样的几何法变得非常有效,并且具有多功能性。 熟练的用户可以在没有不断改变工具的情况下利用长长的直边、大的引导面和刀片质量进行非常广泛的准备。 因此,其学习曲线是新手的局限性,而不是能力有限的证据。

正确的读数从重量、边缘厚度、斜角和制造商的预期用途开始。 普通中国刀对蔬菜和无骨肉,如果被当作骨头工具处理的话,对芯片来说是极好的。 名称不是撞击许可。
...
它是一个受控专家,不是坚不可摧的挡板。 大块稠密的骨头、硬冻材料、不稳定的板块或闪烁的撞击仍然可能造成破坏和伤害。 中风应该是平的,刀片应该拔出而没有侧面杠杆。 薄的纳基里或通用的中国刀片可以分享部分长方形环绕,但其几何学可能完全不适合撞击。
...
目标的移动是漫长的。 让宽阔的尖锐边缘穿过食物, 而不是用许多短短的划线锯。 刀片应该足够长, 足够长, 但不会失去控制 。
...
它的目的仍然是分割熟熟无骨的食物。 较宽的点并不使它成为联合断裂器。 当弯曲的中风在大片熟肉周围感觉自然时选择它; 当喜欢长线画时选择直线的雕刻。
...
其效果是锐利和技巧的次等效应,从来就不是非棍棒刀的保证。 边缘是平坦的,不是悬浮的,而且是在长期控制的抽屉中表现最佳的。 短锯子会浪费设计所提供的长度。
...
当薄的、连续的切片很重要时,设计非常有效,但切片和小板的工作却很尴尬。 它也不是一把小刀:在鱼骨周围紧紧的轮廓下,需要有不同的可控灵活性。
...
传统的例子都是单一的。 这种几何方法可以帮助切片分离,也可以使刀子转向;它通常与普通的双贝片切片不同,手部和刀尖也不同。 它是一个特殊的生鱼专家,而双贝切刀或Sujihiki更能宽恕混合肉类和鱼类的使用。
...
刀片的用意是用无骨鱼干净地画出。 刀片不应该切开关节,也不应该被切开,其构造需要像其他传统的刺刀手一样重视手腕和锋利。
...
这必须区别于现代双贝基吉奥托或K-tip吉奥托作为易懂的全用刀出售。 传统的单贝基通常可以驾驶,通常需要几何化。 光是胶片是不够的;必须说明贝贝的构造。
...
其主要限制是** 缺乏多功能性** 。 狭窄的专门刀片在追查骨骼和连接组织方面非常出色,但很少提供指关节检查,几乎没有食物转移区,而且厨师刀、陀螺或中国刀子所完成的大板工程效率也较低。 因此,应该选择它,因为结扎是一项反复出现的任务,而不是一般用途的替代物。

烧伤意味着在骨头周围工作,而不是切穿它。 扭动点或用它作为杠杆仍然会造成破坏。
...
有些花木使用不对称边缘几何法,因此手势和锐利的建议因模型而异。 其强化的外观不应该被误认为是一把小刀:它发现并分离关节,而不是粉碎大骨头。
...
这与刺片机的硬画不同。 片片子在屠宰期间工作, 在完成部分切开供演示之前。 软体不意味着刀片可以弯曲或弯曲到硬骨上。
...
传统的deba是单击手,其重量并不使它们成为通用的切片:大哺乳动物骨、扭伤或粗心大意的撞击仍然可以使边缘碎裂。 刀片结合了某种鱼壳序列的坚固和精确区域。
...
其狭窄的侧写可以减少拖力,使修剪变得容易,但比起厨师的刀子,它提供的关节通关和食物转移面积要小。 实用性描述的是角色而不是固定长度,因此精确的模型决定它是否感觉更像是长的折叠板刀或紧凑板刀。
...
牙齿继续咬咬人,当表面边缘可能需要维护,并且可以用较少的压力穿透抗药性皮肤时。 它们不会取代面包刀的长度,而磨尖则不那么直截了当。 因此,解释应该从实际模型开始,而不是用终极词开始。
...
同样的曲线在板上提供的直线接触较少。 它不应该被当作微型厨师的刀,或者通过坑、关节和冷冻食品被强迫。 辛祖可以使用普通的Paring Knife标签来标定曲线和直线形式,因此,剖面比单名更重要。
...
与弯曲的对角相比,它不会轻易围绕四舍五入的产品,但是当工作在手和板之间交替时,它更具有多面性。 它的长度和关节清除仍然使得它无法有效进行大规模准备。
...
切口的曲线很小,关节的高度也很少, 所以它仍然是精细的缩略图。 这种形式的来源是完整的 Xinzuo 目录配置, 而不是产品照片; 之所以保留,是因为它识别了该类别, 却没有发明一个不支持的图像 。
...
锋利的平原边缘会留下干净的表面,而血压可能会容忍板块接触的时间更长。 设计上也不是一把磨刀。 刀子不是仅仅因为小而尖锐就用来窥探硬关节的。
...
其形式属于服务和演示,而不是董事会工作。 四舍五入的轮廓看起来是安全的,但确切的产品仍应作为餐具处理,并避免切除、缠绕和杠杆作用。
...
Xinzuo的例子包括为减少拖力而设计的尖锐和松动形式。 开口、空洞和其他松动可以减少连续接触,在奶酪和钢铁之间留有小的空气缺口,但是没有几何方法可以阻止每块奶酪粘在一起。 正确的选择始于质地和部分大小。
...
通过允许个体牙齿** 刺穿食物表面** 来解决这个问题。 用户不是依靠强大的向下压力,而是靠光线绘制或锯锯。 牙齿设置了几个小购买点,用牵引推进刀片,这样碎屑或其他软结构就会用较少的粉碎来切割。

对于今天通常出售的许多面包来说,明显的锯子特征并不总是像以前那样必要:地壳、碎屑结构和分割条件差别很大,非常锋利的平面边缘可以很好地处理一些小面包。 然而,刀片对于非常软的叶子产品** — — 蛋糕、薄饼、薄饼、软卷和类似食品 — — 仍然是必不可少的 — — 在那里,向下压缩可以让产品在直面开始切口之前变形。

使用光线引导锯木运动,让牙齿工作。 过量的压力压碎碎碎屑,使目标落空。 评分长期有效,因为它们的休眠接触较少,但它们需要压缩的磨削机或专家服务,而不是普通的平板划线。
...
这仍然是一个危险的专家操作。 食物必须稳定, 自由手必须保持在路径之外, 刀片移动时没有扭动或撞击。 并非所有冷冻的包件、 嵌入的骨头或大块块块都适合携带此名字的刀子; 并遵循确切的 Xinzuo 规格 。
...
总长度的缩短使得当部分不需要长长的经典雕刻刀时更容易控制。 它的明显曲线支持牛排、熟食部分和其他无骨肉周围的切片和穿透运动,而大刀片比狭窄的切片提供更多的存在和控制。 因此,当厨师想要一种肉类刀时,它特别有用,因为肉类刀比长的雕刻刀更紧凑,机动性更强。

折叠已经到了。 长的雕刻刀对于大型烤肉、火腿或任何连续抽取应该穿过大片部分的任务来说仍然比较有效。 维京刀不是直升机,其独特的轮廓不应该被解释为允许撞击或操纵。

## 选择工作而不是名称

对于大多数厨房来说,第一把严肃的刀应该是厨师的刀子、毛托刀、桑托库刀或中国刀刀,其长度和运动感觉自然。 面包刀和小工具刀或刺刀将主刀的手柄遮盖得不好。 当这些工作经常发生,足以证明专门几何测量是合理的时,长的雕刻、填充、鱼肉和撞击工具就变得值得了。

选择钢之前的配置。 首先决定工作是否需要板接触、 长画、 控制弹性、 精确点或支持质量 。 然后比较该家庭内部的钢、 热处理、 处理和构造 。 此命令防止装饰性或令人印象深刻的名字转移对重要问题的注意力 : 形状是否使预期的移动更清洁、 更安全和更容易控制 ?

## 负责阅读目录名称

Xinzuo目录对当前Xinzuo产品的名称具有权威性,但商业标签并非自动成为普遍的历史类别。 因此,Viking Knife 和 Ultimate Upatlite Knife 仍然被明确确定为 Xinzuo 名称。 Sashimi Knife 与更具体的日本术语相连,而没有默默重命名该产品。 中国的刀和骨头直升机仍然分开,因为相似的矩形轮廓可以隐藏非常不同的厚度、质量和预期撞击工作。

最重要的是,照片可以建立剖面图,而不是硬性、研磨、上传或骨骼能力。这些属性属于精确的模型规格和本指南的后几章。
...
* 来源说明:Xinzuo名称和产品实例来自2025年Xinzuo目录和目前的Xinzuo系列页面,来自目录的产品照片须服从图像权利通知。 PM80-CS视觉作为当前模型识别参考。
